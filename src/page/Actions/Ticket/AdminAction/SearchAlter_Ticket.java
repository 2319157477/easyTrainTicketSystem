 ...
 * @author 23191
 */
public class SearchAlter_Ticket extends JFrame {
    int alter_id = -1;
    ...
    private void Search(ActionEvent e) {
        ...
        if(!search_id_input.getText().isEmpty()) {
            ...
            try {
                ...
                String sql = "SELECT * FROM order_list WHERE order_id = " + search_id;
                System.out.println(sql);
                adminRs = adminState.executeQuery(sql);
                if (adminRs.next()) {
                    c_id = adminRs.getInt(1);
                    user_phone_number = adminRs.getString(2);
    ...
    private void createUIComponents() {
        ...
        try {
            ...
            ResultSet adminRs = trainState.executeQuery("SELECT COUNT(*) FROM order_list");
            if (adminRs.next()) {
                count = adminRs.getInt(1);
            }
            ticket = new Object[count][9];

            adminRs = trainState.executeQuery("SELECT * FROM order_list");
            while (adminRs.next()) {
                c_id = adminRs.getInt(1);
                user_phone_number = adminRs.getString(2);
                train_id = adminRs.getString(3);
    ...
    private void initComponents() {
        ...
        button3.setBounds(new Rectangle(new Point(595, 15), button3.getPreferredSize()));
        ...
        button4.setBounds(new Rectangle(new Point(735, 15), button4.getPreferredSize()));
        ...
        button5.setBounds(new Rectangle(new Point(890, 15), button5.getPreferredSize()));

        contentPane.setPreferredSize(new Dimension(1000, 685));
        pack();
